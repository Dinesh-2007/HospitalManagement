  ...
  return (
    <HospitalTimezoneProvider hname={hname}>
      <HospitalCurrencyProvider hname={hname}>
        <main className="min-h-full bg-white rounded-2xl border dark:bg-transparent px-4 py-5 sm:px-6 lg:px-8">
          ...
          <div className="space-y-6">
            {matchedGroup ? (
              <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div
                  className={`flex min-w-0 max-w-full items-center rounded-2xl bg-slate-200/70 dark:bg-gray-800 p-1.5 ${hasTabOverflow ? "w-full lg:flex-1" : "w-fit"
                    }`}
                >
                  {hasTabOverflow ? (
                    <button
                    ...
                    {matchedGroup.items?.map((item) => {
                      ...
                      return (
                        <Link
                          key={item.href}
                          href={localHref}
                          className={`shrink-0 whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 focus-visible:ring-offset-2 ${isActive
