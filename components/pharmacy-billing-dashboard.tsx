...
export function PharmacyBillingDashboard() {
  ...
  const billingCalculations = useMemo(() => {
    ...
    // Subtotal: perUnit × qty for each line.
    // perUnitPrice may be stored as empty string; fall back to medicineAmount / qty.
    const subtotal = phMedicineLines.reduce((acc, line) => {
      const qty = Number(line.receivedQty || line.prescribedQty || 0);
      const storedPerUnit = Number(line.perUnitPrice || 0);
      const storedTotal  = Number(line.medicineAmount || 0);
      const perUnit = storedPerUnit > 0
        ? storedPerUnit
        : (qty > 0 ? storedTotal / qty : storedTotal);
      ...
      <html>
        ...
        <body>
          ...
          <div style="margin-top: 30px;">
            ...
            <div class="row"><span>Subtotal (Medicines)</span> <span>${currencySymbol}${Number(invoice.subtotal).toFixed(2)}</span></div>
            ${Number((invoice as any).registration_fee) > 0 ? `<div class="row"><span>Additional Fee</span> <span>${currencySymbol}${Number((invoice as any).registration_fee).toFixed(2)}</span></div>` : ''}
                ...
                <table className="min-w-full text-xs text-slate-700 dark:text-slate-300">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-gray-800 bg-white dark:bg-transparent">
                    {phMedicineLines.map((line, idx) => {
                      ...
                      const storedPerUnit = Number(line.perUnitPrice || 0);
                      const storedTotal   = Number(line.medicineAmount || 0);
                      // Derive per-unit price: use stored value or compute from total÷qty
                      const perUnit = storedPerUnit > 0
                        ? storedPerUnit
                        : (qty > 0 ? storedTotal / qty : storedTotal);
                      const lineTotal = perUnit * qty;
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 border-t border-dashed border-slate-200 dark:border-gray-800 space-y-2 text-sm">
                ...
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  ...
                  <span>{formatCurrency(billingCalculations.taxAmount)}</span>
                </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="border-t border-dashed border-gray-200 pt-3 dark:border-gray-800 text-xs space-y-1.5">
                ...
                {Number((viewInvoiceModal as any).registration_fee) > 0 && (
                  <div className="flex justify-between text-gray-500">
                    <span>Additional Fee:</span>
                    <span>{formatCurrency(Number((viewInvoiceModal as any).registration_fee))}</span>
                  </div>
                )}
